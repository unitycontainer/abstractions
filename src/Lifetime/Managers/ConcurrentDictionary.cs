...
namespace System.Collections.Concurrent
{
    ...
    internal class ConcurrentDictionary<TKey, TValue> : IDictionary<TKey, TValue>, IDictionary, IReadOnlyDictionary<TKey, TValue>
    {
        ...
        private class Tables
        {
            ...
            internal readonly IEqualityComparer<TKey> _comparer; // Key equality comparer
        ...
        private bool TryRemoveInternal(TKey key, out TValue value, bool matchValue, TValue oldValue)
        {
            while (true)
            {
                ...
                int bucketNo, lockNo;
                GetBucketAndLockNo(comparer.GetHashCode(key), out bucketNo, out lockNo, tables._buckets.Length, tables._locks.Length);
                ...
                lock (tables._locks[lockNo])
                {
                    ...
                    for (Node curr = tables._buckets[bucketNo]; curr != null; curr = curr.m_next)
                    {
                        ...
                        if (comparer.Equals(curr.m_key, key))
                        {
                            ...
                            if (prev == null)
                            {
                                Volatile.Write<Node>(ref tables._buckets[bucketNo], curr.m_next);
                            }
                            else
                            {
        ...
        public bool TryGetValue(TKey key, out TValue value)
        {
            ...
            Tables tables = _tables;
            IEqualityComparer<TKey> comparer = tables._comparer;
            GetBucketAndLockNo(comparer.GetHashCode(key), out bucketNo, out lockNoUnused, tables._buckets.Length, tables._locks.Length);
        ...
        public void Clear()
        {
            ...
            try
            {
                ...
                Tables newTables = new Tables(new Node[DEFAULT_CAPACITY], _tables._locks, new int[_tables._countPerLock.Length], _tables._comparer);
                _tables = newTables;
        ...
        private void GrowTable(Tables tables, IEqualityComparer<TKey> newComparer, bool regenerateHashKeys, int rehashCount)
        {
            ...
            try
            {
                ...
                _tables = new Tables(newBuckets, newLocks, newCountPerLock, newComparer);
            }
            finally
            {
